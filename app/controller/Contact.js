...
Ext.define('Payback.controller.Contact', {
    ...
    onDataviewItemTap: function(dataview, index, target, record, e, options) {
        ...
        var maxWidth = Ext.Viewport.getWindowWidth()-40;
        var textWidth = header.innerHtmlElement.getWidth();
        var scale = maxWidth/textWidth;
        var x = scale >= 0.9 ? 1:scale;
        var y = 445.686 *x*x  +  -328.785*x + 90.7458;
        header.innerHtmlElement.setStyle({'-webkit-transform': 'scale('+scale+')','-webkit-transform-origin-x': y+'px'});
    },
